...
def compute_hvp(model, loss_fn, dataset, vector, physical_batch_size=None):
    ...
    for data, targets in loader:
        ...
        hvp_batch = torch.autograd.grad(dot_product, model.parameters())
...
def lanczos(hvp_function, dim, neigs=1):
    ...
    return torch.from_numpy(eigenvalues.copy()).float(), torch.from_numpy(eigenvectors.copy()).float()


# Get top eigenvalues of the Hessian
def get_hessian_eigenvalues(model, loss_fn, dataset, neigs=1, physical_batch_size=None):
    num_params = sum(p.numel() for p in model.parameters())
    
    # Create HVP function closure
    hvp_delta = lambda delta: compute_hvp(model, loss_fn, dataset, delta, physical_batch_size)
...
def get_hessian_eigenvalues_and_vectors(model, loss_fn, dataset, neigs=1, physical_batch_size=None):
    num_params = sum(p.numel() for p in model.parameters())
